...
public:
    ...
    public:
        ...
        private:
            ...
            uint32 _checkDisconnectTime;
            uint32 _askMeLaterTime;
            uint32 _sendGossipAgainTime;
        };
        ...
        void RemovePlayerFromQueOnDisconnect(uint32 diff) {
            ...
            if (_checkDisconnectTime + CHECK_PLAYERS_DELAY <= std::time(0)) {
        ...
        void SendGossipMenuToAll() {
            if (_sendGossipMenuTime + CHECK_PLAYERS_DELAY <= std::time(0)) {
                _sendGossipMenuTime = std::time(0);
                for (Player* player : _quedPlayers) {
                }
            }
    ...
    private:
        ...
        uint32 _checkDisconnectTime = CHECK_PLAYERS_DELAY;
